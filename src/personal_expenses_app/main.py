...
from personal_expenses_app.infrastructure.csv_file_loader import CsvFileLoader
...
def pipeline():
    ...
    all_labeled_expenses = csv_file_loader.load_and_label_multiple_files(file_list)
    ml_based_expense_categorizer = MLBasedExpenseCategorizer()
    training_data = ml_based_expense_categorizer.prepare_training_data(
    ...
    for filename in file_list:
        ...
        ml_corrected = corrections.apply_corrections(
            ml_predicted, corrections_df, all_labeled_expenses
        )
        ...
        if not new_corrections.empty:
            ...
            corrections_df = file_persistence.load_corrections(corrections_file)
            ml_corrected = corrections.apply_corrections(
                ml_corrected, corrections_df, all_labeled_expenses
            )
